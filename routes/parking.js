...
router.route('/')
    ...
    .post(isLoggedIn, upload.array('image'), validateParking, catchAsync(parkings.createParking))

router.get('/new', isLoggedIn, parkings.renderNewForm)

router.route('/:id')
    .get(catchAsync(parkings.showParking))
    .put(isLoggedIn, isAuthor, upload.array('image'), validateParking, catchAsync(parkings.updateParking))
