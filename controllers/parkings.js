...
module.exports.createParking = async (req, res) => {
    ...
    parking.images = req.files.map(f => ({ url: f.path, filename: f.filename }))
...
module.exports.updateParking = async (req, res) => {
    const { id } = req.params;
    const parking = await Parking.findByIdAndUpdate(id, { ...req.body.parking }, { new: true });
    ...
    if (req.body.images) {
        ...
        await parking.updateOne({ $pull: { images: { filename: { $in: req.body.deleteImages } } } })
    }
